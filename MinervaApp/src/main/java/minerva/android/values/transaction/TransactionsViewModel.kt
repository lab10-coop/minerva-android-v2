...
class TransactionsViewModel(
    ...
    fun getValue(valueIndex: Int, assetIndex: Int) {
        transactionRepository.getValue(valueIndex, assetIndex)?.let {
            value = it
    ...
    private fun sendSafeAccountAssetTransaction(receiverKey: String, amount: BigDecimal, gasPrice: BigDecimal, gasLimit: BigInteger) {
        launchDisposable {
            val ownerPrivateKey = value.masterOwnerAddress.let { smartContractRepository.getSafeAccountMasterOwnerPrivateKey(it) }
            transactionRepository.resolveENS(receiverKey)
                .flatMap { resolvedENS ->
                    getTransactionForSafeAccount(ownerPrivateKey, resolvedENS, amount, gasPrice, gasLimit)
                        .flatMap {
                            transaction = it
                            smartContractRepository.transferERC20Token(network, it, value.assets[assetIndex].address).toSingleDefault(it)
                        }
                }
    ...
    private fun sendAssetTransaction(receiverKey: String, amount: BigDecimal, gasPrice: BigDecimal, gasLimit: BigInteger) {
        launchDisposable {
            resolveENS(receiverKey, amount, gasPrice, gasLimit, value.assets[assetIndex].address)
                .flatMapCompletable {
                    transactionRepository.transferERC20Token(value.network, it)
                }
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe { _loadingLiveData.value = Event(true) }
    ...
    private fun saveWalletAction(status: Int, transaction: Transaction): Completable =
        walletActionsRepository.saveWalletActions(getValuesWalletAction(transaction, network, status))
